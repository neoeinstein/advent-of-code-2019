...
async fn define_beam(program: intcode::Memory) -> Result<Grid<intcode::Word>> {
    let mut grid = Grid::new(intcode::Word::default(), 50, 50);

    'outer: for col in 0..50 {
        for row in 0..50 {
            ...
            if command.0.send(col).await.is_err() {
                break 'outer;
            }
            if command.0.send(row).await.is_err() {
                break 'outer;
            }
            if let Some(result) = camera.1.recv().await {
                grid.set(GridPosition { row: row as usize, col: col as usize }, result);
            } else {
                break 'outer;
            }
        }
...
pub fn run() -> Result<()> {
    ...
    let grid = runtime.block_on(define_beam(program))?;
