...
//! distances: one tile away (`A`), two tiles away (`B`), three tiles away
//! (`C`), and four tiles away (`D`). If there is ground at the given distance,
//! the register will be `true`; if there is a hole, the register will be
//! `false`.
//!
...
//! and `.` is empty space. For example, suppose you program the springdroid
//! like this:
//!
//! ```text
//! NOT D J
...
pub fn run() -> Result<()> {
    ...
    let term = intcode::AsciiTerminal::new(program);

    let mut runtime = tokio::runtime::Runtime::new()?;
    runtime.block_on(term.execute())?;
